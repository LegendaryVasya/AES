...
def sub_bytes(state):
    # Substitute each byte in the state with its corresponding S-Box value
    for i in range(4):
        for j in range(4):
...
def shift_rows(state):
    # Shift each row of the state cyclically to the left
...
def mix_columns(state):
    ...
    for i in range(4):
        ...
        state[0][i] = (s0 << 1) ^ (s1 ^ (s1 << 1)) ^ s2 ^ s3
        state[1][i] = s0 ^ (s1 << 1) ^ (s2 ^ (s2 << 1)) ^ s3
        state[2][i] = s0 ^ s1 ^ (s2 << 1) ^ (s3 ^ (s3 << 1))
        state[3][i] = (s0 ^ (s0 << 1)) ^ s1 ^ s2 ^ (s3 << 1)
